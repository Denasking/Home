...
import { MOCK_FINES, getFineSummary } from "../data/finesData";

// NEW FEATURE: Mock user session data
const MOCK_USER = {
...
const MOCK_NOTIFICATIONS = [
  ...
  },
  {
    ...
    description: "Your renewed driver's license is ready for collection at the Pretoria DLTC.",
    time: "5 hours ago",
    read: false,
    type: "driver",
  },
  {
    ...
    description: "Your payment of R500.00 for fine reference TF20260001 has been confirmed.",
    time: "1 day ago",
    read: true,
    type: "fine",
  },
...
export default function HomeScreen() {
  ...
  // Get fines summary from shared data
  const finesSummary = getFineSummary(MOCK_FINES);
  ...
  // NEW FEATURE: Handle call action
  const handleCall = () => {
    const phoneNumber = "tel:0670923899";
  ...
  // NEW FEATURE: Handle email action
  const handleEmail = () => {
    const emailUrl = "mailto:choene@gmail.com";
  ...
  // NEW FEATURE: Handle chat (SMS) action
  const handleChat = () => {
    const smsUrl = "sms:0670923899";
    ...
    <SafeAreaView style={styles.safeArea}>
      <View style={styles.wrapper}>
        ...
        >
          ...
          {/* QUICK STATS - Updated with real fines data */}
          ...
          <StatusCard
            icon={<MaterialCommunityIcons name="car-info" size={26} color="#003366" />}
            title="Vehicle License"
            subtitle="BW 21 XB GP • Volkswagen"
            status="Expires in 30 days"
            statusColor="#FF9800"
            onPress={() => navigation.navigate("VehicleLicense")}
          />

          {/* Traffic Fines Card - Updated with real data */}
          <StatusCard
            icon={<MaterialIcons name="warning-amber" size={26} color="#E65100" />}
            title="Traffic Fines"
            subtitle={`Outstanding Balance: R${finesSummary.outstandingBalance} • ${finesSummary.unpaidCount} Unpaid`}
            status={finesSummary.unpaidCount > 0 ? `${finesSummary.unpaidCount} Fine(s) Due` : "Clear"}
            statusColor={finesSummary.unpaidCount > 0 ? "#F44336" : "#4CAF50"}
            onPress={() => navigation.navigate("Fines")}
          />
          
          <StatusCard
            icon={<Ionicons name="calendar-outline" size={24} color="#003366" />}
          ...
          {/* QUICK ACTIONS */}
          <SectionHeader title="Quick Actions" subtitle="Manage your transport needs" />

          <View style={styles.grid}>
            <QuickAction 
              icon="payment" 
              title="Pay Fine" 
              badge={finesSummary.unpaidCount > 0 ? String(finesSummary.unpaidCount) : undefined}
              onPress={() => navigation.navigate("Fines")} 
            />
            <QuickAction 
              icon="sync" 
          ...
          {/* TRAFFIC UPDATES */}
          <SectionHeader title="Traffic Updates" subtitle="Real-time traffic information" />

          <TouchableOpacity style={styles.listItem}>
            <View style={styles.row}>
              <View style={styles.listIconContainer}>
                <MaterialCommunityIcons name="traffic-light" size={22} color="#003366" />
              </View>
          ...
          <TouchableOpacity style={styles.listItem}>
            <View style={styles.row}>
              <View style={styles.listIconContainer}>
                <MaterialCommunityIcons name="road-variant" size={22} color="#003366" />
              </View>
              <View style={styles.listContent}>
          ...
          {/* WEATHER */}
          <SectionHeader title="Weather" subtitle="Current conditions" />
          ...
          {/* FOOTER - Updated with working contact options */}
          <View style={styles.footer}>
            <Text style={styles.footerText}>Need help? Contact support</Text>
            <View style={styles.footerLinks}>
              <TouchableOpacity style={styles.footerLink} onPress={handleCall}>
              ...
              <TouchableOpacity style={styles.footerLink} onPress={handleChat}>
                <Ionicons name="chatbubble-outline" size={18} color="#003366" />
...
function StatusCard({ icon, title, subtitle, status, statusColor = "#4CAF50", onPress }: StatusCardProps) {
  return (
...
const styles = StyleSheet.create({
  ...
  headerLeft: { flex: 1 },
  greeting: { color: "#666", fontSize: 14, fontWeight: "400" },
  userName: { fontSize: 22, fontWeight: "700", color: "#003366", marginTop: 2 },
  ...
  statNumber: { fontSize: 20, fontWeight: "700", color: "#003366", marginTop: 2 },
  ...
  seeAllText: { fontSize: 13, color: "#003366", fontWeight: "500" },
  ...
  actionText: { marginTop: 8, fontWeight: "600", color: "#1a1a1a", fontSize: 13 },
  ...
  listText: { fontSize: 14, fontWeight: "500", color: "#1a1a1a" },
  listSubtext: { fontSize: 12, color: "#999", marginTop: 1 },
  ...
  weatherTemp: { fontSize: 22, fontWeight: "700", color: "#1a1a1a" },
  weatherDesc: { fontSize: 14, color: "#666" },
  weatherDetails: { alignItems: "flex-end" },
  weatherDetail: { flexDirection: "row", alignItems: "center", marginVertical: 2 },
  ...
  notificationText: { fontSize: 14, color: "#555" },
  notificationTextUnread: { fontWeight: "600", color: "#003366" },
  ...
  viewAllText: { fontSize: 14, color: "#003366", fontWeight: "500" },

  footer: { marginTop: 30, alignItems: "center" },
  footerText: { fontSize: 13, color: "#999", marginBottom: 12 },
  footerLinks: { flexDirection: "row", gap: 30 },
  footerLink: { flexDirection: "row", alignItems: "center", gap: 6 },
  footerLinkText: { fontSize: 14, color: "#003366", fontWeight: "500" },
});